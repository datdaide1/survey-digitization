...
>
> **Cập nhật 24/07 — 2 quy tắc rút ra khi review 23 phiếu manual-extraction đầu tiên** (tự phát hiện, không phải phản hồi khách; đã đưa vào [schema/SCHEMA-FORMAT.md §Quy tắc diễn giải đánh dấu](../schema/SCHEMA-FORMAT.md) mục 6/8, [implement-plan-manual-full-record-extraction.md §4](implement-plan-manual-full-record-extraction.md) mục 5/8, và `MARK_RULES` trong [`scripts/lib/mc_extraction.py`](../scripts/lib/mc_extraction.py)): (e) quy tắc "gạch bỏ rồi chọn lại" (b ở trên) áp dụng cả khi 2 dấu xung đột là 1 lựa chọn thường + 1 lựa chọn `exclusive` (case `exclusive_conflict`/`conflicting_answer`) — nếu 1 dấu bị gạch/tô xoá, chỉ lấy dấu còn nguyên, không giữ cả hai/không để trống; (f) câu `single_select` không có tick hình học nhưng có ghi chú lề kiểu "như nhau"/"làm tất cả" chỉ nhiều lựa chọn cùng lúc → coi tất cả là đã chọn, xử lý như `multi_mark_on_single_select` thật.
