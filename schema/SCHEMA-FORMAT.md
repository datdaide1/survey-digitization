...
6. **Gạch bỏ rồi chọn lại (mới 22/07):** nếu thấy 1 dấu tick/X bị gạch bỏ (cancel — có nét gạch chồng lên rõ ràng thể hiện ý huỷ) và có dấu mới ở một lựa chọn khác trên cùng câu → chỉ tính lựa chọn **mới** (dấu đã gạch bỏ không tính là chọn), KHÔNG coi là đa lựa chọn/`multi_mark_on_single_select`. Phân biệt với trường hợp thật sự đa lựa chọn (2 dấu đều còn nguyên, không có nét gạch huỷ nào) — vẫn áp dụng `multi_mark_on_single_select` như case `Q10`/`Q30` đã biết.
