...
from survey_pipeline.providers.anthropic import (
    AnthropicClient,
    ProviderError,
    build_request,
    now_iso,
...
MCExtractionError = ProviderError
ModelOutputError = ProviderError
...
SYSTEM_PROMPT = """Digitize the questionnaire page from the image.
Record only visible evidence, never invent or correct an answer, and use only
codes declared by the supplied schema. When uncertain, use best effort,
confidence=low and needs_review. Call submit_full_page exactly once."""
...
def _field_entry(value_schema: dict[str, Any]) -> dict[str, Any]:
    properties = {"value": value_schema, **_meta_schema()["properties"]}
    return {
        "type": "object", "properties": properties,
        "required": ["value", "confidence", "flags", "note"],
        "additionalProperties": False,
    }
...
def _optional_text_fields(question: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if subfields:
        fields["subfield"] = {
            "type": "object",
            "properties": {item["id"]: _field_entry(_nullable({"type": "string"})) for item in subfields},
    ...
    if derived:
        ...
        fields["derived_subfield"] = {
            "type": "object",
            "properties": {item["id"]: _field_entry(_nullable({"type": "string"})) for item in defs},
...
def answer_schema(question: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if qtype == "composite":
        ...
        properties["components"] = {
            "type": "object",
            "properties": {component["id"]: _field_entry(_scalar_value(component)) for component in components},
        ...
        properties["rows"] = {
            "type": "object",
            "properties": {
                row["code"]: _field_entry(_nullable({"type": "array", "items": {"type": "string", "enum": columns}}))
                for row in rows
            },
...
def _prompt(questionnaire: Mapping[str, Any], page: int) -> str:
    ...
    return (
        ...
        "Tuân thủ hướng dẫn derived_subfield trong schema; không áp quy tắc ngoài schema.\n\n"
...
def extract_page(client: Any, image: Path, questionnaire: Mapping[str, Any], page: int, model: str, max_tokens: int) -> dict[str, Any]:
    ...
    request = build_request(
        ...
        tool=tool, max_tokens=max_tokens, system=SYSTEM_PROMPT,
    )
    return tool_input(client.create_message(request), TOOL_NAME)
...
def _assembly_pages(assembly: Mapping[str, Any], total_pages: int) -> dict[int, Mapping[str, Any]]:
    pages = assembly.get("pages")
    if not isinstance(pages, list):
        raise MCExtractionError("assembly.pages must be an array")
    result: dict[int, Mapping[str, Any]] = {}
    for page in pages:
        if not isinstance(page, Mapping) or not isinstance(page.get("tentative_page"), int):
            raise MCExtractionError("Each assembly page requires an integer tentative_page")
        number = int(page["tentative_page"])
        if number in result:
            raise MCExtractionError(f"Duplicate tentative_page={number}")
        result[number] = page
    expected = set(range(1, total_pages + 1))
    if set(result) != expected:
        raise MCExtractionError(f"Assembly pages {sorted(result)} do not match expected {sorted(expected)}")
    return result


def _resolve_image(image_path: Any, base_dir: str | Path, allowed_roots: Sequence[str | Path] | None) -> Path:
    if not isinstance(image_path, str) or not image_path:
        raise MCExtractionError("Assembly page is missing image_path")
    base = Path(base_dir).resolve()
    path = Path(image_path)
    path = (base / path).resolve() if not path.is_absolute() else path.resolve()
    roots = [Path(root).resolve() for root in (allowed_roots or [base])]
    if not any(path.is_relative_to(root) for root in roots):
        raise MCExtractionError(f"Assembly image is outside allowed roots: {path}")
    if not path.is_file():
        raise MCExtractionError(f"Assembly image does not exist: {path}")
    return path
...
__all__ = ["AnthropicClient", "answer_schema", "build_tool", "extract_record", "merge_runs", "page_questions"]
